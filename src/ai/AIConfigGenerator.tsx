...
const AIConfigAnalyzer: React.FC = () => {
  ...
  useEffect(() => {
    if (analysisResult) {
      showSuccessNotification('Configuration analysis completed successfully!');
    }
  }, [analysisResult]);

  const handleAnalyze = async () => {
    if (!selectedResourceType) {
      showWarningNotification('Please select a resource type');
      return;
    }

    if (!selectedResource) {
      showWarningNotification('Please select a resource to analyze');
      return;
    }

    if (!question.trim()) {
      showWarningNotification('Please enter your question about the configuration');
      return;
    }

    const selectedResourceData = availableResources?.find(r => r.name === selectedResource);
    if (!selectedResourceData) {
      showErrorNotification('Selected resource not found');
      return;
    }

    const resourceTypeInfo = allResourceTypes.find(rt => rt.key === selectedResourceType);
    if (!resourceTypeInfo) {
      showErrorNotification('Resource type information not found');
      return;
    }
    ...
    try {
      ...
      showErrorNotification(err, 'Analysis failed');
    }
  };
