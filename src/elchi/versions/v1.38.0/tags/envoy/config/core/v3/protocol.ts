...
export const QuicProtocolOptions_ConnectionMigrationSettings_MigrateIdleConnectionSettings: OutType = { "QuicProtocolOptions_ConnectionMigrationSettings_MigrateIdleConnectionSettings": [
  {
    "name": "max_idle_time_before_migration",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "Duration",
    "enums": null,
    "comment": "If idle connections are allowed to be migrated, only migrate the connection if it hasn't been idle for longer than this idle period. Otherwise, the connection will be closed instead. Default to 30s.",
    "notImp": false
  }
] };

export const QuicProtocolOptions_ConnectionMigrationSettings_MigrateIdleConnectionSettings_SingleFields = [
  "max_idle_time_before_migration"
];

export const QuicProtocolOptions_ConnectionMigrationSettings: OutType = { "QuicProtocolOptions_ConnectionMigrationSettings": [
  {
    "name": "migrate_idle_connections",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "QuicProtocolOptions_ConnectionMigrationSettings_MigrateIdleConnectionSettings",
    "enums": null,
    "comment": "Config whether and how to migrate idle connections. If absent, idle connections will not be migrated but be closed upon migration signals.",
    "notImp": false
  },
  {
    "name": "max_time_on_non_default_network",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "Duration",
    "enums": null,
    "comment": "After migrating to a non-default network interface, the connection will only be allowed to stay on that network for up to this period of time before being drained unless it migrates to the default network or that network gets picked as the default by the device by then. Default to 128s.",
    "notImp": false
  }
] };

export const QuicProtocolOptions_ConnectionMigrationSettings_SingleFields = [
...
export const QuicProtocolOptions: OutType = { "QuicProtocolOptions": [
  ...
  },
  {
    ...
    "fieldType": "number",
    "enums": null,
    "comment": "Similar to ``initial_stream_window_size``, but for connection-level flow-control. Valid values range from ``1`` to ``25165824`` (``24MB``, maximum supported by QUICHE) and defaults to ``25165824`` (``24 * 1024 * 1024``).\n\n:::note\n\n``16384`` (``2^14``) is the minimum window size supported in Google QUIC. We only support increasing the default window size now, so it's also the minimum.",
    "notImp": false
  },
  {
  ...
  },
  {
    ...
    "enums": null,
    "comment": "Probes the peer at the configured interval to solicit traffic, i.e. ``ACK`` or ``PATH_RESPONSE``, from the peer to push back connection idle timeout. If absent, use the default keepalive behavior of which a client connection sends ``PING``s every ``15s``, and a server connection doesn't do anything.",
    "notImp": false
  },
  {
  ...
  },
  {
    ...
    "fieldType": "Duration",
    "enums": null,
    "comment": "The duration that a QUIC connection stays idle before it closes itself. If this field is not present, QUICHE default ``600s`` will be applied. For internal corporate network, a long timeout is often fine. But for client facing network, ``30s`` is usually a good choice. Do not add an upper bound here. A long idle timeout is useful for maintaining warm connections at non-front-line proxy for low QPS services.",
    "notImp": false
  },
  {
  ...
  {
    "name": "client_packet_writer",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "TypedExtensionConfig",
    "enums": null,
    "comment": "A customized UDP socket and a QUIC packet writer using the socket for client connections. i.e. Mobile uses its own implementation to interact with platform socket APIs. If not present, the default platform-independent socket and writer will be used. extension-category: envoy.quic.client_packet_writer",
    "notImp": false
  },
  {
    ...
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "QuicProtocolOptions_ConnectionMigrationSettings",
    "enums": null,
    "comment": "Enable QUIC `connection migration <https://datatracker.ietf.org/doc/html/rfc9000#name-connection-migration>` to a different network interface when the current network is degrading or has become bad. In order to use a different network interface other than the platform's default one, a customized `client_packet_writer` needs to be configured to create UDP sockets on non-default networks. Only takes effect when runtime key ``envoy.reloadable_features.use_migration_in_quiche`` is true. If absent, the feature will be disabled. [#not-implemented-hide:]",
    "notImp": true
  },
  {
    "name": "memory_reduction_timeout",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "Duration",
    "enums": null,
    "comment": "Timeout for a QUIC connection to schedule memory reduction callback when the network has been idle for a while. This value should be smaller than the idle timeout to take effect. If not specified, memory reduction is set to infinite by QUIC connection (disabled).",
    "notImp": false
  },
  {
    "name": "enable_scone",
    ...
    "fieldType": "boolean",
    "enums": null,
    "comment": "If true, the QUIC connection will signal support for `SCONE <https://datatracker.ietf.org/doc/draft-ietf-scone-protocol/>`_ (Standard Communication with Network Elements) and process SCONE packets. If not present, the QUICHE default behavior will be used.",
    "notImp": false
  }
] };
...
export const HttpProtocolOptions: OutType = { "HttpProtocolOptions": [
  ...
  },
  {
    ...
    "fieldType": "number",
    "enums": null,
    "comment": "The maximum size of response headers. If unconfigured, the default is ``60 KiB``, except for HTTP/1 response headers which have a default of ``80 KiB``. The default value can be overridden by setting runtime key ``envoy.reloadable_features.max_response_headers_size_kb``. Responses that exceed this limit will result in a ``HTTP 503`` response. In Envoy, this setting is only valid when configured on an upstream cluster, not on the `HTTP Connection Manager`.\n\n:::note\n\nCurrently some protocol codecs impose limits on the maximum size of a single header. \n:::\n\n  * HTTP/2 (when using nghttp2) limits a single header to around 100 KB by default. This can be adjusted via `max_header_field_size_kb`. * HTTP/3 limits a single header to around 1024 KB.",
    "notImp": false
  },
  {
...
export const Http1ProtocolOptions_HeaderKeyFormat: OutType = { "Http1ProtocolOptions_HeaderKeyFormat": [
  {
    ...
    "enums": null,
    "comment": "Formats the header by proper casing words: the first character and any character following a special character will be capitalized if it's an alpha character. For example, ``\"content-type\"`` becomes ``\"Content-Type\"``, and ``\"foo$b#$are\"`` becomes ``\"Foo$B#$Are\"``.\n\n:::note\n\nWhile this results in most headers following conventional casing, certain headers are not covered. For example, the ``\"TE\"`` header will be formatted as ``\"Te\"``.",
    "notImp": false
  },
  {
...
export const Http1ProtocolOptions: OutType = { "Http1ProtocolOptions": [
  ...
  },
  {
    ...
    "fieldType": "boolean",
    "enums": null,
    "comment": "Enables trailers for HTTP/1. By default the HTTP/1 codec drops proxied trailers.\n\n:::attention\n\nThis only happens when Envoy is chunk encoding which occurs when: - The request is HTTP/1.1. - Is neither a ``HEAD`` only request nor a HTTP Upgrade. - Not a response to a ``HEAD`` request. - The ``Content-Length`` header is not present.",
    "notImp": false
  },
  {
...
export const KeepaliveSettings: OutType = { "KeepaliveSettings": [
  ...
  },
  {
    ...
    "fieldType": "Duration",
    "enums": null,
    "comment": "How long to wait for a response to a keepalive PING. If a response is not received within this time period, the connection will be aborted.\n\n:::note\n\nIn order to prevent the influence of Head-of-line (HOL) blocking the timeout period is extended when *any* frame is received on the connection, under the assumption that if a frame is received the connection is healthy.",
    "notImp": false
  },
  {
...
export const Http2ProtocolOptions: OutType = { "Http2ProtocolOptions": [
  ...
  },
  {
    ...
    "fieldType": "number",
    "enums": null,
    "comment": "`Initial stream-level flow-control window <https://httpwg.org/specs/rfc7540.html#rfc.section.6.9.2>`_ size. Valid values range from ``65535`` (``2^16 - 1``, HTTP/2 default) to ``2147483647`` (``2^31 - 1``, HTTP/2 maximum) and defaults to ``16MiB`` (``16 * 1024 * 1024``).\n\n:::note\n\n``65535`` is the initial window size from HTTP/2 spec. We only support increasing the default window size now, so it's also the minimum. \n:::\n\nThis field also acts as a soft limit on the number of bytes Envoy will buffer per-stream in the HTTP/2 codec buffers. Once the buffer reaches this pointer, watermark callbacks will fire to stop the flow of data to the codec buffers.",
    "notImp": false
  },
  {
    "name": "initial_connection_window_size",
    ...
    "fieldType": "number",
    "enums": null,
    "comment": "Similar to ``initial_stream_window_size``, but for connection-level flow-control window. The default is ``24MiB`` (``24 * 1024 * 1024``).",
    "notImp": false
  },
  {
    "name": "allow_connect",
  ...
  {
    "name": "enable_huffman_encoding",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "boolean",
    "enums": null,
    "comment": "Controls whether to encode headers using huffman encoding. This can be useful in cases where the cpu spent encoding the headers isn't worth the network bandwidth saved e.g. for localhost. If unset, uses the data plane's default value.",
    "notImp": false
  },
  {
    "name": "max_header_field_size_kb",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "number",
    "enums": null,
    "comment": "Configures the maximum wire-encoded size in KB of an individual header field (name or value) that the ``nghttp2`` HPACK inflater will accept. This limit applies to the HPACK-compressed length on the wire, not the decoded length. If not specified, defaults to ``64`` KB which is the ``nghttp2`` default.\n\nThis limit applies to headers received by the codec. When configured on the downstream HTTP Connection Manager, it limits individual request header fields. When configured on an upstream cluster, it limits individual response header fields.\n\nDue to Huffman encoding, the decoded header size that passes a given wire limit depends on the compression ratio of the content. For example, at the default ``64`` KB wire limit, highly compressible header values can be approximately ``100`` KB when decoded. Increasing this limit allows accepting larger individual headers at the cost of increased memory usage during HPACK decompression.\n\nThis option only applies when using ``nghttp2``. It is a no-op for ``oghttp2``. The configured value of this field sets the per-header field size limit, which must not exceed the applicable aggregate total header size limit. Since a single header field cannot be larger than the total size allowed for all headers combined, this value is validated against `max_request_headers_kb` when configured on the downstream HTTP Connection Manager, and against `max_response_headers_kb` when configured on an upstream cluster.\n\nSince ``Http2ProtocolOptions`` is configured independently for downstream and upstream, different per-header field limits can be set for each direction without requiring separate request and response fields.\n\n:::note\n\nWhen increasing this limit, ensure that upstream services and other proxies in the request path can also handle the larger individual header sizes. Mismatched limits may result in request failures.",
    "notImp": false
  },
  {
    "name": "disallow_obs_text",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "boolean",
    "enums": null,
    "comment": "Whether to disallow obsolete text for oghttp2 in header field values. If not set, it defaults to false. From RFC 9110, https://www.rfc-editor.org/rfc/rfc9110.html#section-5.5: obs-text = %x80-FF",
    "notImp": false
  }
] };
...
export const Http3ProtocolOptions: OutType = { "Http3ProtocolOptions": [
  ...
  },
  {
    ...
    "fieldType": "boolean",
    "enums": null,
    "comment": "Allows proxying Websocket and other upgrades over HTTP/3 CONNECT using the header mechanisms from the `HTTP/2 extended connect RFC <https://datatracker.ietf.org/doc/html/rfc8441>`_ and settings `proposed for HTTP/3 <https://datatracker.ietf.org/doc/draft-ietf-httpbis-h3-websockets/>`_\n\n:::note\n\nHTTP/3 CONNECT is not yet an RFC.",
    "notImp": false
  },
  {
    "name": "allow_metadata",
  ...
  },
  {
    ...
    "fieldType": "boolean",
    "enums": null,
    "comment": "[#not-implemented-hide:] Hiding until Envoy has full HTTP/3 upstream support. Still under implementation. DO NOT USE.\n\nDisables QPACK compression related features for HTTP/3 including: No huffman encoding, zero dynamic table capacity and no cookie crumbling. This can be useful for trading off CPU vs bandwidth when an upstream HTTP/3 connection multiplexes multiple downstream connections.",
    "notImp": true
  },
  {
    "name": "disable_connection_flow_control_for_streams",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "boolean",
    "enums": null,
    "comment": "Disables connection level flow control for HTTP/3 streams. This is useful in situations where the streams share the same connection but originate from different end-clients, so that each stream can make progress independently at non-front-line proxies.",
    "notImp": false
  },
  {
    "name": "disallow_obs_text",
    ...
    "fieldType": "boolean",
    "enums": null,
    "comment": "Whether to disallow obsolete text in header field values. If not set, it defaults to true for alignment with current behavior. As defined in RFC 9110, https://www.rfc-editor.org/rfc/rfc9110.html#section-5.5: an obs-text character is a character in the range %x80-FF",
    "notImp": false
  }
] };
...
export const Http3ProtocolOptions_SingleFields = [
  ...
  "allow_metadata",
  "disable_qpack",
  "disable_connection_flow_control_for_streams",
  "disallow_obs_text"
];
