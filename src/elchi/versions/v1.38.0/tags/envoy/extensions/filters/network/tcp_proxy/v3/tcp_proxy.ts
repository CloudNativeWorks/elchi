...
export const TcpProxy_TcpAccessLogOptions: OutType = { "TcpProxy_TcpAccessLogOptions": [
  ...
  {
    "name": "flush_access_log_on_start",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "boolean",
    "enums": null,
    "comment": "If set to ``true``, the access log is flushed when the TCP proxy accepts a connection.",
    "notImp": false
  }
] };
...
export const TcpProxy: OutType = { "TcpProxy": [
  ...
  },
  {
    ...
    "fieldType": "TlvEntry[]",
    "enums": null,
    "comment": "TLVs to add to the PROXY protocol header sent upstream. Behavior when PROXY protocol state already exists (e.g., downstream TLVs from proxy_protocol listener filter) is controlled by ``proxy_protocol_tlv_merge_policy``.\n\n:::note\nTo ensure the TLVs are allowed upstream, configure passthrough TLVs on the upstream proxy protocol transport. See `core.v3.ProxyProtocolConfig.pass_through_tlvs` for details.",
    "notImp": false
  },
  {
    "name": "proxy_protocol_tlv_merge_policy",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "ProxyProtocolTlvMergePolicy",
    "enums": [
      "ADD_IF_ABSENT",
      "OVERWRITE_BY_TYPE_IF_EXISTS_OR_ADD",
      "APPEND_IF_EXISTS_OR_ADD"
    ],
    "comment": "Specifies how TLVs in ``proxy_protocol_tlvs`` are merged with existing PROXY protocol state (e.g., downstream TLVs from the proxy_protocol listener filter). See `ProxyProtocolTlvMergePolicy`.",
    "notImp": false
  },
  {
