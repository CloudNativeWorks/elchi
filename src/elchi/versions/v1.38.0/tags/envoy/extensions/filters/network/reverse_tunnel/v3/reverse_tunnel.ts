...
export const Validation: OutType = { "Validation": [
  ...
  },
  {
    ...
    "fieldType": "string",
    "enums": null,
    "comment": "Format string to extract the expected tenant identifier for validation. The formatted value is compared against the ``x-envoy-reverse-tunnel-tenant-id`` header from the incoming handshake request. If they do not match, the connection is rejected with HTTP ``403 Forbidden``.\n\nSupports the same `command operators` as ``node_id_format``.\n\nIf empty, tenant ID validation is skipped.",
    "notImp": false
  },
  {
...
export const ReverseTunnel: OutType = { "ReverseTunnel": [
  ...
  },
  {
    ...
    "enums": null,
    "comment": "Optional validation configuration for node, cluster, and tenant identifiers. If specified, the filter validates the ``x-envoy-reverse-tunnel-node-id``, ``x-envoy-reverse-tunnel-cluster-id``, and ``x-envoy-reverse-tunnel-tenant-id`` headers against expected values extracted using format strings. Requests that fail validation are rejected with HTTP ``403 Forbidden``.",
    "notImp": false
  },
  {
