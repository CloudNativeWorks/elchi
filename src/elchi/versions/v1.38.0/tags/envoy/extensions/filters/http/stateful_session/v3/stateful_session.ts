...
export const StatefulSession: OutType = { "StatefulSession": [
  ...
  {
    "name": "status_on_strict_destination_not_found",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "number",
    "enums": null,
    "comment": "The HTTP status code to return when ``strict`` mode is enabled and the requested destination is not found in the set of available endpoints. This does not apply when the destination exists but is unhealthy. This field has no effect when ``strict`` is set to ``false`` and will be ignored. Defaults to ``503`` (Service Unavailable) if not specified or set to ``0``.",
    "notImp": false
  }
] };
