...
export const protobufPackage = "envoy.config.filter.listener.original_src.v2alpha1";
...
export interface OriginalSrc {
  $type: "envoy.config.filter.listener.original_src.v2alpha1.OriginalSrc";
  /**
...
function createBaseOriginalSrc(): OriginalSrc {
  return { $type: "envoy.config.filter.listener.original_src.v2alpha1.OriginalSrc" };
}

export const OriginalSrc: MessageFns<OriginalSrc, "envoy.config.filter.listener.original_src.v2alpha1.OriginalSrc"> = {
  $type: "envoy.config.filter.listener.original_src.v2alpha1.OriginalSrc" as const,

  encode(message: OriginalSrc, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.bind_port !== undefined && message.bind_port !== false) {
      writer.uint32(8).bool(message.bind_port);
    }
    if (message.mark !== undefined && message.mark !== 0) {
      writer.uint32(16).uint32(message.mark);
    }
    return writer;
  },
  ...
  fromJSON(object: any): OriginalSrc {
    return {
      $type: OriginalSrc.$type,
      bind_port: isSet(object.bind_port) ? globalThis.Boolean(object.bind_port) : undefined,
  ...
  toJSON(message: OriginalSrc): unknown {
    const obj: any = {};
    if (message.bind_port !== undefined) {
      obj.bind_port = message.bind_port;
    }
  ...
  fromPartial<I extends Exact<DeepPartial<OriginalSrc>, I>>(object: I): OriginalSrc {
    const message = createBaseOriginalSrc();
    message.bind_port = object.bind_port ?? undefined;
