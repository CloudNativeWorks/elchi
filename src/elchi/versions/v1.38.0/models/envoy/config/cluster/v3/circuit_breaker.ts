...
export interface CircuitBreakers {
   ...
   */
  per_host_thresholds?: CircuitBreakers_Thresholds[] | undefined;
}
...
function createBaseCircuitBreakers(): CircuitBreakers {
  return { $type: "envoy.config.cluster.v3.CircuitBreakers" };
}

export const CircuitBreakers: MessageFns<CircuitBreakers, "envoy.config.cluster.v3.CircuitBreakers"> = {
  $type: "envoy.config.cluster.v3.CircuitBreakers" as const,
  ...
  encode(message: CircuitBreakers, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.per_host_thresholds !== undefined && message.per_host_thresholds.length !== 0) {
      for (const v of message.per_host_thresholds) {
        CircuitBreakers_Thresholds.encode(v!, writer.uint32(18).fork()).join();
      }
    }
    return writer;
  },
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 2: {
          ...
          if (message.per_host_thresholds === undefined) {
            message.per_host_thresholds = [];
          }
          const el = CircuitBreakers_Thresholds.decode(reader, reader.uint32());
          if (el !== undefined) {
            message.per_host_thresholds!.push(el);
          }
          continue;
        }
      }
  ...
  fromJSON(object: any): CircuitBreakers {
    return {
      ...
      thresholds: globalThis.Array.isArray(object?.thresholds)
        ...
        : undefined,
      per_host_thresholds: globalThis.Array.isArray(object?.per_host_thresholds)
        ? object.per_host_thresholds.map((e: any) => CircuitBreakers_Thresholds.fromJSON(e))
        : undefined,
    };
  },
  ...
  toJSON(message: CircuitBreakers): unknown {
    ...
    if (message.per_host_thresholds?.length) {
      obj.per_host_thresholds = message.per_host_thresholds.map((e) => CircuitBreakers_Thresholds.toJSON(e));
    }
    return obj;
  },
  ...
  fromPartial<I extends Exact<DeepPartial<CircuitBreakers>, I>>(object: I): CircuitBreakers {
    ...
    message.per_host_thresholds = object.per_host_thresholds?.map((e) => CircuitBreakers_Thresholds.fromPartial(e)) ||
      undefined;
    return message;
  },
};
...
function createBaseCircuitBreakers_Thresholds(): CircuitBreakers_Thresholds {
  return { $type: "envoy.config.cluster.v3.CircuitBreakers.Thresholds" };
}
...
> = {
  $type: "envoy.config.cluster.v3.CircuitBreakers.Thresholds" as const,
...
function createBaseCircuitBreakers_Thresholds_RetryBudget(): CircuitBreakers_Thresholds_RetryBudget {
  return { $type: "envoy.config.cluster.v3.CircuitBreakers.Thresholds.RetryBudget" };
}
...
> = {
  $type: "envoy.config.cluster.v3.CircuitBreakers.Thresholds.RetryBudget" as const,
