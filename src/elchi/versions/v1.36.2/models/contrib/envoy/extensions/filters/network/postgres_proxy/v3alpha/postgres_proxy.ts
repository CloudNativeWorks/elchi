...
export interface PostgresProxy {
   ...
   * Defaults to ``DISABLE``.
   */
  downstream_ssl?: PostgresProxy_SSLMode | undefined;
}

/** Downstream and Upstream SSL operational modes. */
export enum PostgresProxy_SSLMode {
  /**
   * DISABLE - If used in downstream ssl, do not terminate SSL session initiated by a client.
  ...
  DISABLE = "DISABLE",
  /**
   * REQUIRE - If used in downstream ssl, the Postgres proxy filter will terminate SSL
  ...
  REQUIRE = "REQUIRE",
  /**
   * ALLOW - If used in downstream SSL, the Postgres proxy filter will accept downstream
...
export function postgresProxy_SSLModeFromJSON(object: any): PostgresProxy_SSLMode {
  switch (object) {
    ...
    case 2:
    case "ALLOW":
      return PostgresProxy_SSLMode.ALLOW;
    default:
...
export function postgresProxy_SSLModeToJSON(object: PostgresProxy_SSLMode): string {
  switch (object) {
    ...
    case PostgresProxy_SSLMode.REQUIRE:
      return "REQUIRE";
    case PostgresProxy_SSLMode.ALLOW:
      return "ALLOW";
    default:
      throw new globalThis.Error("Unrecognized enum value " + object + " for enum PostgresProxy_SSLMode");
  }
...
> = {
  ...
  encode(message: PostgresProxy, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.downstream_ssl !== undefined && message.downstream_ssl !== PostgresProxy_SSLMode.DISABLE) {
      writer.uint32(40).int32(postgresProxy_SSLModeToNumber(message.downstream_ssl));
    }
    return writer;
  },
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 5: {
          ...
          message.downstream_ssl = postgresProxy_SSLModeFromJSON(reader.int32());
          continue;
        }
      }
      if ((tag & 7) === 4 || tag === 0) {
  ...
  fromJSON(object: any): PostgresProxy {
    return {
      ...
      downstream_ssl: isSet(object.downstream_ssl) ? postgresProxy_SSLModeFromJSON(object.downstream_ssl) : undefined,
    };
  },
  ...
  toJSON(message: PostgresProxy): unknown {
    ...
    if (message.downstream_ssl !== undefined) {
      obj.downstream_ssl = postgresProxy_SSLModeToJSON(message.downstream_ssl);
    }
    return obj;
  },
  ...
  fromPartial<I extends Exact<DeepPartial<PostgresProxy>, I>>(object: I): PostgresProxy {
    ...
    message.downstream_ssl = object.downstream_ssl ?? undefined;
    return message;
  },
};
