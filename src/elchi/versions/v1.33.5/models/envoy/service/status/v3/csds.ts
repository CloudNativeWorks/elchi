...
export enum ConfigStatus {
  ...
  NOT_SENT = "NOT_SENT",
  /**
   * STALE - Management server has sent the config to client but hasn’t received
   * ACK/NACK.
   */
  STALE = "STALE",
  /**
...
export enum ClientConfigStatus {
  ...
  /**
   * CLIENT_RECEIVED_ERROR - Client received an error from the control plane. The attached config
   * dump is the most recent accepted one. If no config is accepted yet,
   * the attached config dump will be empty.
   */
  CLIENT_RECEIVED_ERROR = "CLIENT_RECEIVED_ERROR",
}
...
export function clientConfigStatusFromJSON(object: any): ClientConfigStatus {
  switch (object) {
    ...
    case 4:
    case "CLIENT_RECEIVED_ERROR":
      return ClientConfigStatus.CLIENT_RECEIVED_ERROR;
    default:
...
export function clientConfigStatusToJSON(object: ClientConfigStatus): string {
  switch (object) {
    ...
    case ClientConfigStatus.CLIENT_RECEIVED_ERROR:
      return "CLIENT_RECEIVED_ERROR";
    default:
      throw new globalThis.Error("Unrecognized enum value " + object + " for enum ClientConfigStatus");
  }
}
