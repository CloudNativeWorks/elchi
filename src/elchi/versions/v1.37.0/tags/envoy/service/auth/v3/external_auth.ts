...
export const CheckResponse: OutType = { "CheckResponse": [
  ...
  {
    "name": "http_response.error_response",
    "isUnion": true,
    "isDeprecated": false,
    "fieldType": "DeniedHttpResponse",
    "enums": null,
    "comment": "Supplies http attributes for an error response. This is used when the authorization service encounters an internal error and wants to return custom headers and body to the downstream client. When ``error_response`` is set, the ext_authz filter increments the ``ext_authz_error`` stat and respects the `failure_mode_allow` configuration. The HTTP status code, headers, and body are taken from the `DeniedHttpResponse` message. If the status field is not set, Envoy sends the status code configured via `status_on_error`, which defaults to ``403 Forbidden``.",
    "notImp": false
  },
  {
    "name": "dynamic_metadata",
