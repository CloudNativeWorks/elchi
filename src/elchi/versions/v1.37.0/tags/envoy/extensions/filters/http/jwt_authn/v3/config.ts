...
export const JwtRequirement: OutType = { "JwtRequirement": [
  ...
  },
  {
    ...
    "fieldType": "Empty",
    "enums": null,
    "comment": "The requirement is always satisfied even if JWT is missing or the JWT verification fails. A typical usage is: this filter is used to only verify JWTs and pass the verified JWT payloads to another filter, the other filter will make decision. In this mode, all JWTs will be verified.",
    "notImp": false
  },
  {
  ...
  },
  {
    ...
    "enums": null,
    "comment": "Extract JWT claims without performing signature validation. This mode will decode the JWT, extract claims, and forward them as configured (via claim_to_headers, forward_payload_header, etc.) but will NOT verify the JWT signature against JWKS.\n\n:::warning\n\nThis mode does not verify JWT authenticity. Use only in scenarios where: \n:::\n\n   - JWTs come from a trusted source (e.g., internal service mesh) - Signature verification is performed elsewhere in the request path - You are in a testing period and the token issuer doesn't support JWKS yet\n\nThis mode will:\n\n* Decode the JWT header and payload * Extract claims and forward them as headers * Always return success (Status::Ok) regardless of JWT validity * Log when extraction occurs\n\nThis mode will NOT:\n\n* Verify the JWT signature * Validate the (issuer) claim * Validate the (audience) claim * Check not-before time (nbf claim)",
    "notImp": false
  }
] };
