 ...
 * [#not-implemented-hide:]
 */
export interface LbEndpointCollection {
  $type: "envoy.config.endpoint.v3.LbEndpointCollection";
...
function createBaseLbEndpointCollection(): LbEndpointCollection {
  return { $type: "envoy.config.endpoint.v3.LbEndpointCollection" };
}

export const LbEndpointCollection: MessageFns<LbEndpointCollection, "envoy.config.endpoint.v3.LbEndpointCollection"> = {
  $type: "envoy.config.endpoint.v3.LbEndpointCollection" as const,

  encode(message: LbEndpointCollection, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): LbEndpointCollection {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseLbEndpointCollection();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
  ...
  fromJSON(object: any): LbEndpointCollection {
    return {
      $type: LbEndpointCollection.$type,
      entries: isSet(object.entries) ? CollectionEntry.fromJSON(object.entries) : undefined,
    };
  },

  toJSON(message: LbEndpointCollection): unknown {
    const obj: any = {};
    if (message.entries !== undefined) {
  ...
  create<I extends Exact<DeepPartial<LbEndpointCollection>, I>>(base?: I): LbEndpointCollection {
    return LbEndpointCollection.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<LbEndpointCollection>, I>>(object: I): LbEndpointCollection {
    const message = createBaseLbEndpointCollection();
...
messageTypeRegistry.set(LbEndpointCollection.$type, LbEndpointCollection);
