...
export const BufferSettings: OutType = { "BufferSettings": [
  {
    ...
    "fieldType": "number",
    "enums": null,
    "comment": "Sets the maximum size of a message body that the filter will hold in memory. Envoy will return ``HTTP 413`` and will *not* initiate the authorization process when the buffer reaches the size set in this field. Note that this setting will have precedence over `failure_mode_allow`.",
    "notImp": false
  },
  {
...
export const ExtAuthz: OutType = { "ExtAuthz": [
  ...
  },
  {
    ...
    "fieldType": "boolean",
    "enums": null,
    "comment": "Clears the route cache in order to allow the external authorization service to correctly affect routing decisions. The filter clears all cached routes when:\n\n#. The field is set to ``true``.\n\n#. The status returned from the authorization service is an HTTP 200 or gRPC 0.\n\n#. At least one ``authorization response header`` is added to the client request, or is used to alter another client request header.",
    "notImp": false
  },
  {
...
export const AuthorizationRequest: OutType = { "AuthorizationRequest": [
  ...
  },
  {
    ...
    "enums": null,
    "comment": "Sets a list of headers that will be included in the request to the authorization service. Note that client request headers with the same key will be overridden.",
    "notImp": false
  }
] };
