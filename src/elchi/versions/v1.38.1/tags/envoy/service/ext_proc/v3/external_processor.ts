...
export const ProtocolConfiguration: OutType = { "ProtocolConfiguration": [
  {
    ...
    "comment": "Specifies the filter configuration `request_body_mode`.",
    "notImp": false
  },
  {
    ...
    "comment": "Specifies the filter configuration `response_body_mode`.",
    "notImp": false
  },
  {
    ...
    "fieldType": "boolean",
    "enums": null,
    "comment": "Specifies the filter configuration `send_body_without_waiting_for_header_response`. If the client is waiting for a header response from the server, setting to ``true`` means the client will send the body to the server as it arrives. Setting to ``false`` means the client will buffer the arrived data and not send it to the server immediately.",
    "notImp": false
  }
] };
...
export const ProcessingRequest: OutType = { "ProcessingRequest": [
  {
    ...
    "enums": null,
    "comment": "This represents the different types of messages that the data plane can send to an external processing server. [#next-free-field: 12]",
    "notImp": false
  },
  {
...
export const ProcessingResponse: OutType = { "ProcessingResponse": [
  {
    ...
    "enums": null,
    "comment": "This represents the different types of messages the server may send back to the data plane when the ``observability_mode`` field in the received ``ProcessingRequest`` is set to ``false``.\n\n* If the corresponding ``BodySendMode`` in the `processing_mode` is not set to ``FULL_DUPLEX_STREAMED``, then for every received ``ProcessingRequest``, the server must send back exactly one ``ProcessingResponse`` message. * If it is set to ``FULL_DUPLEX_STREAMED``, the server must follow the API defined for this mode to send the ``ProcessingResponse`` messages. [#next-free-field: 13]",
    "notImp": false
  },
  {
  ...
  },
  {
    ...
    "enums": null,
    "comment": "Override how parts of the HTTP request and response are processed for the duration of this particular request/response only. Servers may use this to intelligently control how requests are processed based on the headers and other metadata that they see.\n\nThis field is only applicable when servers are responding to the header requests. If it is set in the response to the body or trailer requests, it will be ignored by the data plane. It is also ignored by the data plane when the ext_proc filter config `allow_mode_override` is set to ``false``, or `send_body_without_waiting_for_header_response` is set to ``true``.",
    "notImp": false
  },
  {
    "name": "request_drain",
  ...
  },
  {
    ...
    "fieldType": "Duration",
    "enums": null,
    "comment": "When the ext_proc server receives a request message and needs more time to process it, it sends back a ``ProcessingResponse`` message with a new timeout value. When the data plane receives this response message, it ignores other fields in the response, stops the original timer (which has the timeout value specified in `message_timeout`), and starts a new timer with this ``override_message_timeout`` value while keeping the data plane ext_proc filter state machine intact.\n\nThe value must be >= 1ms and <= `max_message_timeout`. Such a message can be sent at most once in a particular data plane ext_proc filter processing state. To enable this API, ``max_message_timeout`` must be set to a value >= 1ms.",
    "notImp": false
  }
] };
...
export const HttpHeaders: OutType = { "HttpHeaders": [
  ...
  },
  {
    ...
    "fieldType": "Map<string, { [key: string]: any; }>",
    "enums": null,
    "comment": "[#not-implemented-hide:] This field is deprecated and not implemented. Attributes will be sent in the top-level `attributes` field.",
    "notImp": true
  },
  {
    "name": "end_of_stream",
...
export const BodyMutation: OutType = { "BodyMutation": [
  {
    ...
    "fieldType": "Uint8Array<ArrayBufferLike>",
    "enums": null,
    "comment": "This message specifies the body mutation the server sends to the data plane.",
    "notImp": false
  },
  {
...
export const CommonResponse: OutType = { "CommonResponse": [
  ...
  },
  {
    ...
    "enums": null,
    "comment": "Replace the body of the last message sent to the remote server on this stream. If responding to an ``HttpBody`` request, simply replace or clear the body chunk that was sent with that request. Body mutations may take effect in response either to ``header`` or ``body`` messages. When it is in response to ``header`` messages, it only takes effect if the `status` is set to ``CONTINUE_AND_REPLACE``.",
    "notImp": false
  },
  {
    "name": "trailers",
    ...
    "fieldType": "HeaderMap",
    "enums": null,
    "comment": "[#not-implemented-hide:] Add new trailers to the message. This may be used when responding to either an ``HttpHeaders`` or ``HttpBody`` message, but only if this message is returned along with the ``CONTINUE_AND_REPLACE`` status. The header value is encoded in the `raw_value` field.",
    "notImp": true
  },
  {
...
export const StreamedBodyResponse: OutType = { "StreamedBodyResponse": [
  ...
  },
  {
    ...
    "fieldType": "boolean",
    "enums": null,
    "comment": "The server sets this flag to ``true`` if it has received a body request with `end_of_stream` set to ``true``, and this is the last chunk of body responses.\n\nNote that in ``GRPC`` body send mode, this allows the ext_proc server to tell the data plane to send a half close after a client message, which will result in discarding any other messages sent by the client application.",
    "notImp": false
  },
  {
    "name": "end_of_stream_without_message",
