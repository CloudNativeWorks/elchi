...
export const CaresDnsResolverConfig: OutType = { "CaresDnsResolverConfig": [
  ...
  {
    "name": "reinit_channel_on_timeout",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "boolean",
    "enums": null,
    "comment": "If true, reinitialize the c-ares channel when a DNS query fails with ``ARES_ETIMEOUT``.\n\nThis can help recover from rare cases where the UDP sockets held by the c-ares channel become unusable after timeouts, causing subsequent queries to fail or Envoy to keep serving stale DNS results. When enabled, a timeout-triggered reinitialization attempts to restore healthy state quickly. In environments where timeouts are caused by intermittent network issues, enabling this may increase channel churn; consider using `max_udp_channel_duration` for periodic refresh instead.\n\nDefault is false.",
    "notImp": false
  }
] };
