...
export const HttpConnectionManager_Tracing: OutType = { "HttpConnectionManager_Tracing": [
  ...
  {
    "name": "no_context_propagation",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "boolean",
    "enums": null,
    "comment": "If set to true, trace context propagation is disabled, meaning that trace context headers (e.g. ``traceparent``, ``tracestate`` for OpenTelemetry/W3C, or ``X-B3-*`` headers for Zipkin) will not be injected when proxying requests to upstreams.\n\nThis is useful for scenarios where you want to report spans from a proxy (e.g., an egress gateway) while preventing trace context from being propagated to external services, effectively stopping the trace at the mesh boundary.\n\nNote that span reporting is still performed when this is set to true - only context propagation is disabled.\n\nDefault: false (context propagation is enabled)",
    "notImp": false
  }
] };
...
export const HttpConnectionManager_SetCurrentClientCertDetails: OutType = { "HttpConnectionManager_SetCurrentClientCertDetails": [
  ...
  },
  {
    ...
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "HttpConnectionManager_ForwardClientCertFormat",
    "enums": [
      "TEXT",
      "JSON"
    ],
    "comment": "The format for the header. When the `forward_client_cert_details` is APPEND_FORWARD and an existing XFCC header is present, the format of the existing header is used. The configured format is used when there is no existing header value (APPEND_FORWARD with no prior XFCC header, or SANITIZE_SET which always replaces the value).",
    "notImp": false
  }
] };
...
export const HttpConnectionManager_ProxyStatusConfig: OutType = { "HttpConnectionManager_ProxyStatusConfig": [
  ...
  },
  {
    ...
    "fieldType": "boolean",
    "enums": null,
    "comment": "Configures the manner in which the Proxy-Status HTTP response header is populated.\n\nSee the [Proxy-Status RFC](https://datatracker.ietf.org/doc/html/draft-ietf-httpbis-proxy-status-08). \n\nThe Proxy-Status header is a string of the form:\n\n  \"<server_name>; error=<error_type>; details=<details>\" [#next-free-field: 7]",
    "notImp": false
  },
  {
...
export const ForwardProtoConfig: OutType = { "ForwardProtoConfig": [
  {
    "name": "https_destination_ports",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "number[]",
    "enums": null,
    "comment": "List of destination ports that should be treated as HTTPS. When the PROXY protocol destination port matches one of these ports, ``x-forwarded-proto`` will be set to ``https``.\n\nCommon values: 443, 8443",
    "notImp": false
  },
  {
    "name": "http_destination_ports",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "number[]",
    "enums": null,
    "comment": "List of destination ports that should be treated as HTTP. When the PROXY protocol destination port matches one of these ports, ``x-forwarded-proto`` will be set to ``http``.\n\nCommon values: 80, 8080",
    "notImp": false
  }
] };

export const ForwardProtoConfig_SingleFields = [
  "https_destination_ports",
...
export const HttpConnectionManager: OutType = { "HttpConnectionManager": [
  ...
  },
  {
    ...
    "enums": null,
    "comment": "Additional settings for HTTP requests handled by the connection manager. These will be applicable to both HTTP/1.1 and HTTP/2 requests.",
    "notImp": false
  },
  {
    ...
    "fieldType": "boolean",
    "enums": null,
    "comment": "If set to ``true``, Envoy will not initiate an immediate drain timer for downstream HTTP/1 connections once `common_http_protocol_options.max_connection_duration` is exceeded. Instead, Envoy will wait until the next downstream request arrives, add a ``connection: close`` header to the response, and then gracefully close the connection once the stream has completed.\n\nThis behavior adheres to `RFC 9112, Section 9.6 <https://www.rfc-editor.org/rfc/rfc9112#name-tear-down>`_.\n\nIf set to ``false``, exceeding ``max_connection_duration`` triggers Envoy's default drain behavior for HTTP/1, where the connection is eventually closed after all active streams finish.\n\nThis option has no effect if ``max_connection_duration`` is not configured. Defaults to ``false``.",
    "notImp": false
  },
  {
  ...
  },
  {
    ...
    "fieldType": "Matcher",
    "enums": null,
    "comment": "The matcher for forwarding client cert details. This allows per-request configuration of forward client cert behavior based on request properties. If a matcher is configured and matches a request, the matched action's forward client cert config will be used. If the matcher is not configured or doesn't match, the static `forward_client_cert_details` and `set_current_client_cert_details` config will be used as fallback.\n\nExample: If the x-forwarded-client-cert header contains \"trusted-client\", use APPEND_FORWARD, otherwise use SANITIZE_SET:\n\n```yaml\n\n  forward_client_cert_matcher:\n    matcher_list:\n      matchers:\n      - predicate:\n          single_predicate:\n            input:\n              name: envoy.matching.inputs.request_headers\n              typed_config:\n                \"@type\": type.googleapis.com/envoy.type.matcher.v3.HttpRequestHeaderMatchInput\n                header_name: \"x-forwarded-client-cert\"\n            value_match:\n              string_match:\n                contains: \"trusted-client\"\n        on_match:\n          action:\n            name: forward_client_cert\n            typed_config:\n              \"@type\": type.googleapis.com/envoy.extensions.filters.network.http_connection_manager.v3.HttpConnectionManager.ForwardClientCertConfig\n              forward_client_cert_details: APPEND_FORWARD\n              set_current_client_cert_details:\n                uri: true\n    on_no_match:\n      action:\n        name: forward_client_cert\n        typed_config:\n          \"@type\": type.googleapis.com/envoy.extensions.filters.network.http_connection_manager.v3.HttpConnectionManager.ForwardClientCertConfig\n          forward_client_cert_details: SANITIZE_SET\n          set_current_client_cert_details:\n            uri: true",
    "notImp": false
  },
  {
  ...
  {
    "name": "forward_proto_config",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "ForwardProtoConfig",
    "enums": null,
    "comment": "Configuration for controlling how the ``x-forwarded-proto`` header is set. This allows customization of protocol inference, including support for inferring the original protocol (HTTP or HTTPS) from the PROXY protocol destination port.\n\nThis is useful when a Layer 4 load balancer (such as AWS NLB) terminates TLS and uses PROXY protocol to communicate with Envoy.\n\nWhen configured and the local address was restored from PROXY protocol (indicating the original destination address is available), the ``x-forwarded-proto`` header will be set based on whether the destination port is in ``https_destination_ports`` or ``http_destination_ports``.\n\nExample configuration:\n\n```yaml\n\n  http_connection_manager:\n    forward_proto_config:\n      https_destination_ports: [443, 8443]\n      http_destination_ports: [80, 8080]\n```\n\nIf not configured, defaults to disabled and the standard behavior applies (using connection TLS status or trusted downstream headers).",
    "notImp": false
  }
] };
...
export const HttpConnectionManager_ForwardClientCertConfig: OutType = { "HttpConnectionManager_ForwardClientCertConfig": [
  {
    "name": "forward_client_cert_details",
    "isUnion": false,
    "isDeprecated": false,
    ...
    "comment": "How to handle the XFCC header.",
    "notImp": false
  },
  {
    "name": "set_current_client_cert_details",
    ...
    "fieldType": "HttpConnectionManager_SetCurrentClientCertDetails",
    "enums": null,
    "comment": "The fields in the client certificate to forward. See `set_current_client_cert_details` for details.",
    "notImp": false
  }
] };

export const HttpConnectionManager_ForwardClientCertConfig_SingleFields = [
  "forward_client_cert_details"
];
...
export const ScopedRoutes_ScopeKeyBuilder_FragmentBuilder: OutType = { "ScopedRoutes_ScopeKeyBuilder_FragmentBuilder": [
  {
    ...
    "enums": null,
    "comment": "Specifies the mechanism for constructing key fragments which are composed into scope keys.",
    "notImp": false
  }
] };
...
export const ScopedRoutes_ScopeKeyBuilder_FragmentBuilder_HeaderValueExtractor: OutType = { "ScopedRoutes_ScopeKeyBuilder_FragmentBuilder_HeaderValueExtractor": [
  ...
  },
  {
    ...
    "fieldType": "number",
    "enums": null,
    "comment": "Specifies how the value of a header should be extracted. The following example maps the structure of a header to the fields in this message.\n\n.. code::\n\n             <0> <1>   <-- index X-Header: a=b;c=d |         || | |         || \\----> <element_separator> |         || |         |\\----> <element.separator> |         | |         \\----> <element.key> | \\----> <name>\n\n   Each 'a=b' key-value pair constitutes an 'element' of the header field.",
    "notImp": false
  },
  {
