...
export const ExtAuthz: OutType = { "ExtAuthz": [
  ...
  },
  {
    ...
    "fieldType": "boolean",
    "enums": null,
    "comment": "When set to ``true``, the filter operates in shadow mode. In shadow mode the filter still calls the external authorization service and processes the response, but never terminates the request. Instead of sending a local reply on a denied or error response, the filter writes the authorization decision (engine result, status code, response headers) into the request's `FilterState` as a `ShadowDecision` object so that subsequent filters can read and optionally enforce it.\n\nThe FilterState key is the filter's configured ``name`` in the filter chain with a ``.shadow`` suffix (``envoy.filters.http.ext_authz.shadow`` by default). Multiple ext_authz filters in the same chain must already have distinct names and therefore write to distinct keys automatically.\n\nThe auth server's denied-response body is intentionally **not** carried on the ShadowDecision: bodies can be arbitrarily large and no downstream consumer in the shadow-comparison flow needs them. A consumer that wants to reproduce the auth server's full denied response must read it from its own source of truth rather than replaying it from FilterState.\n\nHeader and query-parameter mutations from an OK response are still applied to the request as usual.\n\nDefaults to ``false``.",
    "notImp": false
  }
] };
...
export const ShadowDecision: OutType = { "ShadowDecision": [
  {
    "name": "check_result",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "ShadowDecision_CheckResult",
    "enums": [
      "UNSPECIFIED",
      "OK",
      "DENIED",
    ...
    "comment": "The decision the auth server returned.",
    "notImp": false
  },
  {
    "name": "status_code",
    ...
    "fieldType": "number",
    "enums": null,
    "comment": "Response status code associated with the decision. For ``DENIED`` and ``ERROR`` this is the code the filter would have set on termination (the auth server's code for ``DENIED``, or `status_on_error` fallback for ``ERROR``). For ``OK`` this defaults to ``200`` so consumers always see a populated value — the upstream response code is not known at shadow-decision time.",
    "notImp": false
  },
  {
    ...
    "enums": null,
    "comment": "Response headers the auth server asked to set on a denied response (e.g. ``WWW-Authenticate``, ``Set-Cookie``). Populated for ``DENIED`` only. Preserves ordering and duplicate header names.",
    "notImp": false
  }
] };

export const ShadowDecision_SingleFields = [
  "check_result",
  "status_code"
];
...
export const HttpService: OutType = { "HttpService": [
  ...
  },
  {
    ...
    "fieldType": "string",
    "enums": null,
    "comment": "Sets a prefix to the value of authorization request header ``Path``. Only one of ``path_prefix`` or ``path_override`` may be set.",
    "notImp": false
  },
  {
    "name": "path_override",
    ...
    "fieldType": "string",
    "enums": null,
    "comment": "Replaces the value of authorization request header ``Path`` with this value. Only one of ``path_prefix`` or ``path_override`` may be set.",
    "notImp": false
  },
  {
...
export const ExtAuthzPerRoute: OutType = { "ExtAuthzPerRoute": [
  {
    ...
    "fieldType": "boolean",
    "enums": null,
    "comment": "Extra settings on a per virtualhost/route/weighted-cluster level.",
    "notImp": false
  },
  {
...
export const CheckSettings: OutType = { "CheckSettings": [
  ...
  },
  {
    ...
    "fieldType": "GrpcService",
    "enums": null,
    "comment": "Extra settings for the check request. [#next-free-field: 6]",
    "notImp": false
  },
  {
