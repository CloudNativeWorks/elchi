...
export const Admin: OutType = { "Admin": [
  ...
  },
  {
    ...
    "enums": null,
    "comment": "List of admin paths that are accessible. If not specified, all admin endpoints are accessible. Matchers are evaluated against the request path. For endpoints commonly queried with parameters (for example ``/stats?format=...``), prefer ``prefix`` matchers.\n\nWhen specified, only paths in this list will be accessible, all others will return ``HTTP 403 Forbidden``.\n\nExample:\n\n```yaml\n\n  allow_paths:\n  - prefix: /stats\n  - prefix: /config_dump\n  - exact: /ready\n  - prefix: /healthcheck",
    "notImp": false
  }
] };
...
export const MemoryAllocatorManager: OutType = { "MemoryAllocatorManager": [
  ...
  {
    "name": "soft_memory_limit_bytes",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "number",
    "enums": null,
    "comment": "Sets the soft memory limit for tcmalloc. When the total memory used by tcmalloc exceeds this limit, background release will be performed more aggressively to bring memory usage below the limit. If not set, no soft memory limit is applied.\n\n:::note\nThis is currently only supported with tcmalloc and not with ``gperftools``.",
    "notImp": false
  },
  {
    "name": "max_per_cpu_cache_size_bytes",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "number",
    "enums": null,
    "comment": "Sets the maximum per-CPU cache size in bytes for tcmalloc. Smaller values reduce per-CPU memory overhead at the cost of increased contention on the central free list. If not set, tcmalloc's default is used.\n\n:::note\nThis is currently only supported with tcmalloc and not with ``gperftools``.",
    "notImp": false
  },
  {
    "name": "max_unfreed_memory_bytes",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "number",
    "enums": null,
    "comment": "The threshold of unfreed memory in bytes that triggers the heap shrinker to release memory back to the OS. When the difference between physical memory used and application-allocated memory exceeds this threshold, free memory is released.\n\nDefaults to ``104857600`` (100 MB).",
    "notImp": false
  }
] };
...
export const Bootstrap: OutType = { "Bootstrap": [
  ...
  },
  {
    ...
    "fieldType": "boolean",
    "enums": null,
    "comment": "Bootstrap `configuration overview`. [#next-free-field: 43]",
    "notImp": false
  },
  {
