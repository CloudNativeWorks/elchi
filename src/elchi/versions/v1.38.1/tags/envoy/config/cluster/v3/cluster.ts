...
export const Cluster: OutType = { "Cluster": [
  ...
  {
    "name": "transport_socket_matcher",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "Matcher",
    "enums": null,
    "comment": "Optional matcher that selects a transport socket from `transport_socket_matches`.\n\nThis matcher uses the generic xDS matcher framework to select a named transport socket based on various inputs available at transport socket selection time.\n\nSupported matching inputs:\n\n* ``endpoint_metadata``: Extract values from the selected endpoint's metadata. * ``locality_metadata``: Extract values from the endpoint's locality metadata. * ``transport_socket_filter_state``: Extract values from filter state that was explicitly shared from downstream to upstream via ``TransportSocketOptions``. This enables flexible downstream-connection-based matching, such as:\n\n  - Network namespace matching. - Custom connection attributes. - Any data explicitly passed via filter state.\n\n:::note\nFilter state sharing follows the same pattern as tunneling in Envoy. Filters must explicitly share data by setting filter state with the appropriate sharing mode. The filter state is then accessible via the ``transport_socket_filter_state`` input during transport socket selection. \n:::\n\nIf this field is set, it takes precedence over legacy metadata-based selection performed by `transport_socket_matches` alone. If the matcher does not yield a match, Envoy uses the default transport socket configured for the cluster.\n\nWhen using this field, each entry in `transport_socket_matches` must have a unique ``name``. The matcher outcome is expected to reference one of these names.",
    "notImp": false
  },
  {
    "name": "name",
  ...
  },
  {
    ...
    "comment": "Configuration for a single upstream cluster. [#next-free-field: 61]",
    "notImp": false
  },
  {
  ...
  },
  {
    ...
    "fieldType": "Duration",
    "enums": null,
    "comment": "Optional timeout that controls how long an upstream connection is allowed to stay above the configured buffer high watermark before it is closed. If this timeout is not specified, or explicitly set to 0, connections will not be closed due to buffer high watermark usage.",
    "notImp": false
  },
  {
    "name": "lb_policy",
