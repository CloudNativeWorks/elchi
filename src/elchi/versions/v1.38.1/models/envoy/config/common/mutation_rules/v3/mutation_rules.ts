...
export interface HeaderMutation {
  ...
  action?: { $case: "remove"; remove: string } | { $case: "append"; append: HeaderValueOption } | {
    $case: "remove_on_match";
    remove_on_match: HeaderMutation_RemoveOnMatch;
  } | undefined;
}

export interface HeaderMutation_RemoveOnMatch {
  $type: "envoy.config.common.mutation_rules.v3.HeaderMutation.RemoveOnMatch";
  /**
   * A string matcher that will be applied to the header key. If the header key
   * matches, the header will be removed.
   */
  key_matcher?: StringMatcher | undefined;
}
...
export const HeaderMutation: MessageFns<HeaderMutation, "envoy.config.common.mutation_rules.v3.HeaderMutation"> = {
  ...
  encode(message: HeaderMutation, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    switch (message.action?.$case) {
      ...
      case "append":
        ...
        break;
      case "remove_on_match":
        HeaderMutation_RemoveOnMatch.encode(message.action.remove_on_match, writer.uint32(26).fork()).join();
        break;
    }
    return writer;
  },
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 3: {
          ...
          message.action = {
            $case: "remove_on_match",
            remove_on_match: HeaderMutation_RemoveOnMatch.decode(reader, reader.uint32()),
          };
          continue;
        }
      }
  ...
  fromJSON(object: any): HeaderMutation {
    return {
      ...
      action: isSet(object.remove)
        ...
        : isSet(object.remove_on_match)
        ? { $case: "remove_on_match", remove_on_match: HeaderMutation_RemoveOnMatch.fromJSON(object.remove_on_match) }
        : undefined,
    };
  },
  ...
  toJSON(message: HeaderMutation): unknown {
    ...
    if (message.action?.$case === "remove_on_match") {
      obj.remove_on_match = HeaderMutation_RemoveOnMatch.toJSON(message.action.remove_on_match);
    }
    return obj;
  },
  ...
  fromPartial<I extends Exact<DeepPartial<HeaderMutation>, I>>(object: I): HeaderMutation {
    ...
    if (
      object.action?.$case === "remove_on_match" &&
      object.action?.remove_on_match !== undefined &&
      object.action?.remove_on_match !== null
    ) {
      message.action = {
        $case: "remove_on_match",
        remove_on_match: HeaderMutation_RemoveOnMatch.fromPartial(object.action.remove_on_match),
      };
    }
    return message;
  },
...
function createBaseHeaderMutation_RemoveOnMatch(): HeaderMutation_RemoveOnMatch {
  return { $type: "envoy.config.common.mutation_rules.v3.HeaderMutation.RemoveOnMatch" };
}

export const HeaderMutation_RemoveOnMatch: MessageFns<
  HeaderMutation_RemoveOnMatch,
  "envoy.config.common.mutation_rules.v3.HeaderMutation.RemoveOnMatch"
> = {
  $type: "envoy.config.common.mutation_rules.v3.HeaderMutation.RemoveOnMatch" as const,

  encode(message: HeaderMutation_RemoveOnMatch, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.key_matcher !== undefined) {
      StringMatcher.encode(message.key_matcher, writer.uint32(10).fork()).join();
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): HeaderMutation_RemoveOnMatch {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseHeaderMutation_RemoveOnMatch();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
          ...
          message.key_matcher = StringMatcher.decode(reader, reader.uint32());
          continue;
        }
      }
      if ((tag & 7) === 4 || tag === 0) {
  ...
  fromJSON(object: any): HeaderMutation_RemoveOnMatch {
    return {
      $type: HeaderMutation_RemoveOnMatch.$type,
      key_matcher: isSet(object.key_matcher) ? StringMatcher.fromJSON(object.key_matcher) : undefined,
    };
  },

  toJSON(message: HeaderMutation_RemoveOnMatch): unknown {
    const obj: any = {};
    if (message.key_matcher !== undefined) {
      obj.key_matcher = StringMatcher.toJSON(message.key_matcher);
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<HeaderMutation_RemoveOnMatch>, I>>(base?: I): HeaderMutation_RemoveOnMatch {
    return HeaderMutation_RemoveOnMatch.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<HeaderMutation_RemoveOnMatch>, I>>(object: I): HeaderMutation_RemoveOnMatch {
    const message = createBaseHeaderMutation_RemoveOnMatch();
    message.key_matcher = (object.key_matcher !== undefined && object.key_matcher !== null)
      ? StringMatcher.fromPartial(object.key_matcher)
      : undefined;
    return message;
  },
};

messageTypeRegistry.set(HeaderMutation_RemoveOnMatch.$type, HeaderMutation_RemoveOnMatch);
