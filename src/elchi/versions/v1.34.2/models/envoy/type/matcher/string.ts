...
function createBaseStringMatcher(): StringMatcher {
  return { $type: "envoy.type.matcher.StringMatcher", match_pattern: undefined };
}

export const StringMatcher: MessageFns<StringMatcher, "envoy.type.matcher.StringMatcher"> = {
  $type: "envoy.type.matcher.StringMatcher" as const,
  ...
  encode(message: StringMatcher, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    switch (message.match_pattern?.$case) {
      ...
      case "suffix":
        ...
        break;
      case "regex":
        writer.uint32(34).string(message.match_pattern.regex);
        break;
      case "safe_regex":
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 4: {
          ...
          message.match_pattern = { $case: "regex", regex: reader.string() };
          continue;
        }
        case 5: {
          if (tag !== 42) {
  ...
  toJSON(message: StringMatcher): unknown {
    ...
    if (message.match_pattern?.$case === "regex") {
      obj.regex = message.match_pattern.regex;
    }
  ...
  fromPartial<I extends Exact<DeepPartial<StringMatcher>, I>>(object: I): StringMatcher {
    ...
    if (
      object.match_pattern?.$case === "regex" &&
      object.match_pattern?.regex !== undefined &&
      object.match_pattern?.regex !== null
    ) {
      message.match_pattern = { $case: "regex", regex: object.match_pattern.regex };
    }
    if (
...
function createBaseListStringMatcher(): ListStringMatcher {
  return { $type: "envoy.type.matcher.ListStringMatcher" };
}

export const ListStringMatcher: MessageFns<ListStringMatcher, "envoy.type.matcher.ListStringMatcher"> = {
  $type: "envoy.type.matcher.ListStringMatcher" as const,
