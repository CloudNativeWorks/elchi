 ...
 */
export interface NetworkNamespaceInput {
  $type: "envoy.extensions.matching.common_inputs.network.v3.NetworkNamespaceInput";
}
...
function createBaseNetworkNamespaceInput(): NetworkNamespaceInput {
  return { $type: "envoy.extensions.matching.common_inputs.network.v3.NetworkNamespaceInput" };
}

export const NetworkNamespaceInput: MessageFns<
  NetworkNamespaceInput,
  "envoy.extensions.matching.common_inputs.network.v3.NetworkNamespaceInput"
> = {
  $type: "envoy.extensions.matching.common_inputs.network.v3.NetworkNamespaceInput" as const,

  encode(_: NetworkNamespaceInput, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): NetworkNamespaceInput {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseNetworkNamespaceInput();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
      }
  ...
  fromJSON(_: any): NetworkNamespaceInput {
    return { $type: NetworkNamespaceInput.$type };
  },

  toJSON(_: NetworkNamespaceInput): unknown {
    const obj: any = {};
    return obj;
  },

  create<I extends Exact<DeepPartial<NetworkNamespaceInput>, I>>(base?: I): NetworkNamespaceInput {
    return NetworkNamespaceInput.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<NetworkNamespaceInput>, I>>(_: I): NetworkNamespaceInput {
    const message = createBaseNetworkNamespaceInput();
    return message;
  },
};

messageTypeRegistry.set(NetworkNamespaceInput.$type, NetworkNamespaceInput);
