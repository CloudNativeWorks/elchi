...
export interface SocketStreamedTraceSegment {
  ...
  message_piece?: { $case: "connection"; connection: Connection } | { $case: "event"; event: SocketEvent } | {
    $case: "events";
    events: SocketEvents;
  } | undefined;
}
...
export const SocketEvent: MessageFns<SocketEvent, "envoy.data.tap.v3.SocketEvent"> = {
  ...
  encode(message: SocketEvent, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.seq_num !== undefined && message.seq_num !== 0) {
      writer.uint32(48).uint64(message.seq_num);
    }
    return writer;
  },
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 6: {
          ...
          message.seq_num = longToNumber(reader.uint64());
          continue;
        }
      }
      if ((tag & 7) === 4 || tag === 0) {
  ...
  fromJSON(object: any): SocketEvent {
    return {
      ...
      seq_num: isSet(object.seq_num) ? globalThis.Number(object.seq_num) : undefined,
    };
  },
  ...
  toJSON(message: SocketEvent): unknown {
    ...
    if (message.seq_num !== undefined) {
      obj.seq_num = Math.round(message.seq_num);
    }
    return obj;
  },
