...
export const EndpointHealth: OutType = { "EndpointHealth": [
  ...
  },
  {
    ...
    "enums": null,
    "comment": "Optional metadata about the health check result, populated by the active health checker and forwarded to the management server for richer health state interpretation.\n\nWell-known keys:\n\n``http_status_code`` (number) Set by the HTTP health checker. Contains the HTTP response status code returned by the upstream endpoint during the most recent health check, e.g. ``200``, ``503``. Only present when the health check received a complete HTTP response; absent on connection failures or timeouts.",
    "notImp": false
  }
] };
