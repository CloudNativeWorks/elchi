...
export const MetadataOptions: OutType = { "MetadataOptions": [
  {
    ...
    "enums": null,
    "comment": "Describes which typed or untyped filter dynamic metadata namespaces to forward to the external processing server.",
    "notImp": false
  },
  {
    ...
    "fieldType": "MetadataOptions_MetadataNamespaces",
    "enums": null,
    "comment": "Describes which typed or untyped filter dynamic metadata namespaces to accept from the external processing server. Set to empty or leave unset to disallow writing any received dynamic metadata. Receiving of typed metadata is not supported.",
    "notImp": false
  },
  {
    "name": "cluster_metadata_forwarding_namespaces",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "MetadataOptions_MetadataNamespaces",
    "enums": null,
    "comment": "Describes which cluster metadata namespaces to forward to the external processing server.:::note\nThis is the least specific metadata. Should there be any namespace collision, cluster level metadata can be overridden by filter metadata.",
    "notImp": false
  }
] };
...
export const ExternalProcessor: OutType = { "ExternalProcessor": [
  {
    ...
    "enums": null,
    "comment": "Configuration for the gRPC service that the filter will communicate with. Only one of ``grpc_service`` or ``http_service`` can be set. It is required that one of them must be set.",
    "notImp": false
  },
  {
    "name": "http_service",
  ...
  },
  {
    ...
    "fieldType": "string[]",
    "enums": null,
    "comment": "The data plane provides a number of `attributes` for expressive policies. Each attribute name provided in this field will be matched against that list and populated in the `ProcessingRequest.attributes` field. See the `attribute documentation` for the list of supported attributes and their types.",
    "notImp": false
  },
  {
  ...
  },
  {
    ...
    "fieldType": "boolean",
    "enums": null,
    "comment": "If ``allow_mode_override`` is set to true, the filter config `processing_mode` can be overridden by the response message from the external processing server `mode_override`. If not set, ``mode_override`` API in the response message will be ignored. Mode override is not supported if the body send mode is ``FULL_DUPLEX_STREAMED``.",
    "notImp": false
  },
  {
  ...
  },
  {
    ...
    "fieldType": "boolean",
    "enums": null,
    "comment": "If true, send each part of the HTTP request or response specified by ``ProcessingMode`` without pausing on filter chain iteration. It is \"Send and Go\" mode that can be used by external processor to observe the request's data and status. In this mode:\n\n1. Only ``STREAMED``, ``GRPC``, and ``NONE`` body processing modes are supported; for any other body processing mode, the body will not be sent.\n\n2. External processor should not send back processing response, as any responses will be ignored. This also means that `message_timeout` restriction doesn't apply to this mode.\n\n3. External processor may still close the stream to indicate that no more messages are needed.",
    "notImp": false
  },
  {
  ...
  },
  {
    ...
    "enums": null,
    "comment": "Decorator to introduce custom logic that runs after the ``ProcessingRequest`` is constructed, but before it is sent to the External Processor. The ``ProcessingRequest`` may be modified.\n\n:::note\nProcessing request modifiers are currently in alpha. \n:::\n\nextension-category: envoy.http.ext_proc.processing_request_modifiers",
    "notImp": false
  },
  {
  ...
  },
  {
    ...
    "enums": null,
    "comment": "Sets the HTTP status code that is returned to the client when the external processing server returns an error, fails to respond, or cannot be reached.\n\nThe default status is ``HTTP 500 Internal Server Error``.",
    "notImp": false
  },
  {
    "name": "allow_content_length_header",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "boolean",
    "enums": null,
    "comment": "If true, the filter will not remove the ``content-length`` header from the request/response after external processing. It is typically used in `FULL_DUPLEX_STREAMED` mode. If the original body has been modified, the external processing server needs to set the correct content-length header in HeaderMutation that matches the modified body length.\n\n:::warning\n\nThis configuration should only be used if you are sure that the content length matches the body length after external processing. Otherwise, it may cause vulnerability issues such as request smuggling. Thus, please use your own discretion when enabling this feature.",
    "notImp": false
  }
] };
...
export const ExtProcPerRoute: OutType = { "ExtProcPerRoute": [
  {
    ...
    "fieldType": "boolean",
    "enums": null,
    "comment": "Extra settings that may be added to per-route configuration for a virtual host or cluster.",
    "notImp": false
  },
  {
...
export const ExtProcOverrides: OutType = { "ExtProcOverrides": [
  ...
  },
  {
    ...
    "fieldType": "boolean",
    "enums": null,
    "comment": "[#not-implemented-hide:] Set a different asynchronous processing option than the default. Deprecated and not implemented.",
    "notImp": true
  },
  {
    "name": "request_attributes",
  ...
  },
  {
    ...
    "fieldType": "TypedExtensionConfig",
    "enums": null,
    "comment": "Decorator to introduce custom logic that runs after the ``ProcessingRequest`` is constructed, but before it is sent to the External Processor. The ``ProcessingRequest`` may be modified. This is a per-route override of `processing_request_modifier`.",
    "notImp": false
  }
] };
