...
export const OutlierDetection: OutType = { "OutlierDetection": [
  ...
  {
    "name": "detect_degraded_hosts",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "boolean",
    "enums": null,
    "comment": "If set to true, outlier detection will mark hosts as degraded when they return the ``x-envoy-degraded`` header. Degraded hosts are deprioritized in load balancing but are not ejected from the cluster. The degraded state is cleared using the same backoff algorithm as ejection, with the degradation period calculated as ``base_ejection_time`` multiplied by the number of times the host has been marked as degraded, capped by ``max_ejection_time``. Defaults to false.",
    "notImp": false
  }
] };
