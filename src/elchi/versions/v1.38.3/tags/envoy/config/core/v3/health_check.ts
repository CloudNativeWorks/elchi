...
export const HealthCheck_Payload: OutType = { "HealthCheck_Payload": [
  {
    ...
    "fieldType": "string",
    "enums": null,
    "comment": "Describes the encoding of the payload bytes in the payload.",
    "notImp": false
  },
  {
    "name": "payload.binary",
...
export const HealthCheck_HttpHealthCheck: OutType = { "HealthCheck_HttpHealthCheck": [
  ...
  },
  {
    ...
    "enums": null,
    "comment": "HTTP specific payload to be sent as the request body during health checking. If specified, the method should support a request body (POST, PUT, PATCH, etc.).",
    "notImp": false
  },
  {
    "name": "receive",
  ...
  },
  {
    ...
    "comment": "HTTP Method that will be used for health checking, default is \"GET\". GET, HEAD, POST, PUT, DELETE, OPTIONS, TRACE, PATCH methods are supported. Request body payloads are supported for POST, PUT, PATCH, and OPTIONS methods only. CONNECT method is disallowed because it is not appropriate for health check request. If a non-200 response is expected by the method, it needs to be set in `expected_statuses`.",
    "notImp": false
  }
] };
