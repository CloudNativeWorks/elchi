...
export const GrpcService: OutType = { "GrpcService": [
  {
    ...
    "enums": null,
    "comment": "gRPC service configuration. This is used by `ApiConfigSource` and filter configurations. [#next-free-field: 7]",
    "notImp": false
  },
  {
  ...
  },
  {
    ...
    "enums": null,
    "comment": "Optional default retry policy for RPCs or streams initiated toward this gRPC service.\n\nIf an async stream does not have a retry policy configured in its per‑stream options, this policy is used as the default.\n\n:::note\n\nThis field is only applied by Envoy gRPC (``envoy_grpc``) clients. Google gRPC (``google_grpc``) clients currently ignore this field. \n:::\n\nIf not specified, no default retry policy is applied at the client level and retries only occur when explicitly configured in per‑stream options.",
    "notImp": false
  }
] };
...
export const GrpcService_EnvoyGrpc: OutType = { "GrpcService_EnvoyGrpc": [
  ...
  },
  {
    ...
    "fieldType": "RetryPolicy",
    "enums": null,
    "comment": "Specifies the retry backoff policy for re-establishing long‑lived xDS gRPC streams.\n\nThis field is optional. If ``retry_back_off.max_interval`` is not provided, it will be set to ten times the configured ``retry_back_off.base_interval``.\n\n:::note\n\nThis field is only honored for management‑plane xDS gRPC streams created from `ApiConfigSource` that use ``envoy_grpc``. Data‑plane gRPC clients (for example external authorization or external processing filters) must use `GrpcService.retry_policy` instead. \n:::\n\nIf not set, xDS gRPC streams default to a base interval of 500ms and a maximum interval of 30s.",
    "notImp": false
  },
  {
