...
export const AdditionalAddress: OutType = { "AdditionalAddress": [
  ...
  },
  {
    ...
    "enums": null,
    "comment": "Configures TCP keepalive settings for the additional address. If not set, the listener `tcp_keepalive` configuration is inherited. You can explicitly disable TCP keepalive for the additional address by setting any keepalive field (`keepalive_probes`, `keepalive_time`, or `keepalive_interval`) to ``0``.",
    "notImp": false
  }
] };
...
export const Listener_FcdsConfig: OutType = { "Listener_FcdsConfig": [
  {
    "name": "name",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "string",
    "enums": null,
    "comment": "Optional name to present to the filter chain discovery service. This may be an arbitrary name with arbitrary length. If a name is not provided, the listener's name is used. Refer to `filter_chains`. for details on how listener name is determined if unspecified. In addition, this may be a xdstp:// URL.",
    "notImp": false
  },
  {
    "name": "config_source",
    ...
    "enums": null,
    "comment": "Configuration for the source of FCDS updates for this listener.:::note\nThis discovery service only supports ``AGGREGATED_GRPC`` API type.",
    "notImp": false
  }
] };

export const Listener_FcdsConfig_SingleFields = [
  "name"
];
...
export const Listener_ConnectionBalanceConfig: OutType = { "Listener_ConnectionBalanceConfig": [
  {
    ...
    "enums": null,
    "comment": "Configuration for listener connection balancing.",
    "notImp": false
  },
  {
...
export const Listener: OutType = { "Listener": [
  ...
  {
    "name": "per_connection_buffer_high_watermark_timeout",
    "isUnion": false,
    "isDeprecated": false,
    "fieldType": "Duration",
    "enums": null,
    "comment": "Optional timeout that controls how long a connection is allowed to stay above the configured buffer high watermark before it is closed. If this timeout is not specified, or explicitly set to 0, connections will not be closed due to buffer high watermark usage.",
    "notImp": false
  },
  {
    "name": "metadata",
    ...
    "fieldType": "Metadata",
    "enums": null,
    "comment": "Listener metadata.\n\nThe following pre-defined metadata could be used by Envoy to manipulate the listener behavior:\n\n* ``envoy.stats_matcher``: this metadata could be used to customize the stats emitted by the listener. See `well-known metadata` for more details.",
    "notImp": false
  },
  {
    "name": "deprecated_v1",
  ...
  },
  {
    ...
    "enums": null,
    "comment": "Additional socket options that may not be present in Envoy source code or precompiled binaries. It is not allowed to update the socket options for any existing address if `enable_reuse_port` is ``false`` to avoid the conflict when creating new sockets for the listener.",
    "notImp": false
  },
  {
  ...
  },
  {
    ...
    "fieldType": "TcpKeepalive",
    "enums": null,
    "comment": "If set, TCP keepalive settings are configured for the listener address and inherited by additional addresses. If not set, TCP keepalive settings are not configured for the listener address and additional addresses by default. See `tcp_keepalive` to explicitly configure TCP keepalive settings for individual additional addresses.",
    "notImp": false
  }
] };
