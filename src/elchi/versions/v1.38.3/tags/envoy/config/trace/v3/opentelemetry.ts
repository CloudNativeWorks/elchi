...
export const OpenTelemetryConfig: OutType = { "OpenTelemetryConfig": [
  ...
  },
  {
    ...
    "enums": null,
    "comment": "The upstream HTTP cluster that will receive OTLP traces. This field can be left empty to disable reporting traces to the HTTP service. Only one of ``grpc_service``, ``http_service`` may be used.\n\n:::note\n\nThe ``request_headers_to_add`` property in the OTLP HTTP exporter service supports substitution formatters. The formatters cannot access any HTTP or connection properties, but can load content such as environment variables or files or secrets.",
    "notImp": false
  },
  {
    "name": "service_name",
  ...
  },
  {
    ...
    "fieldType": "number",
    "enums": null,
    "comment": "Envoy caches the span in memory when the OpenTelemetry backend service is temporarily unavailable. This field specifies the maximum number of spans that can be cached. If not specified, the default is 1024.",
    "notImp": false
  }
] };
