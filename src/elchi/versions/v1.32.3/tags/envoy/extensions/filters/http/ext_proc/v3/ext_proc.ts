...
export const ExternalProcessor: OutType = { "ExternalProcessor": [
  ...
  },
  {
    ...
    "enums": null,
    "comment": "[#not-implemented-hide:] Configuration for the HTTP service that the filter will communicate with. Only one of ``http_service`` or `grpc_service`. can be set. It is required that one of them must be set.\n\nIf ``http_service`` is set, the `processing_mode` can not be configured to send any body or trailers. i.e, http_service only supports sending request or response headers to the side stream server.\n\nWith this configuration, Envoy behavior:\n\n1. The headers are first put in a proto message `ProcessingRequest`.\n\n2. This proto message is then transcoded into a JSON text.\n\n3. Envoy then sends a HTTP POST message with content-type as \"application/json\", and this JSON text as body to the side stream server.\n\nAfter the side-stream receives this HTTP request message, it is expected to do as follows:\n\n1. It converts the body, which is a JSON string, into a ``ProcessingRequest`` proto message to examine and mutate the headers.\n\n2. It then sets the mutated headers into a new proto message `ProcessingResponse`.\n\n3. It converts ``ProcessingResponse`` proto message into a JSON text.\n\n4. It then sends a HTTP response back to Envoy with status code as \"200\", content-type as \"application/json\" and sets the JSON text as the body.",
    "notImp": true
  },
  {
  ...
  },
  {
    ...
    "comment": "Specifies the action to be taken when an external processor response is received in response to request headers. It is recommended to set this field than set `disable_clear_route_cache`. Only one of ``disable_clear_route_cache`` or ``route_cache_action`` can be set.",
    "notImp": false
  },
  {
  ...
  },
  {
    ...
    "fieldType": "ProcessingMode[]",
    "enums": null,
    "comment": "When `allow_mode_override` is enabled and ``allowed_override_modes`` is configured, the filter config `processing_mode` can only be overridden by the response message from the external processing server iff the `mode_override` is allowed by the ``allowed_override_modes`` allow-list below.",
    "notImp": false
  }
] };
