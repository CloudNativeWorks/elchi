...
export const Bootstrap_StaticResources: OutType = { "Bootstrap_StaticResources": [
  ...
  },
  {
    ...
    "fieldType": "Cluster[]",
    "enums": null,
    "comment": "If a network based configuration source is specified for `cds_config`, it's necessary to have some initial cluster definitions available to allow Envoy to know how to speak to the management server. These cluster definitions may not use `EDS` (i.e. they should be static IP or DNS-based).",
    "notImp": false
  },
  {
    "name": "secrets",
...
export const Bootstrap_DeferredStatOptions: OutType = { "Bootstrap_DeferredStatOptions": [
  {
    ...
    "fieldType": "boolean",
    "enums": null,
    "comment": "When the flag is enabled, Envoy will lazily initialize a subset of the stats (see below). This will save memory and CPU cycles when creating the objects that own these stats, if those stats are never referenced throughout the lifetime of the process. However, it will incur additional memory overhead for these objects, and a small increase of CPU usage when a at least one of the stats is updated for the first time. Groups of stats that will be lazily initialized: - Cluster traffic stats: a subgroup of the `cluster statistics` that are used when requests are routed to the cluster.",
    "notImp": false
  }
] };
...
export const Bootstrap: OutType = { "Bootstrap": [
  ...
  },
  {
    ...
    "enums": null,
    "comment": "Optional gRPC async manager config.",
    "notImp": false
  },
  {
...
export const RuntimeLayer_RtdsLayer: OutType = { "RuntimeLayer_RtdsLayer": [
  {
    ...
    "fieldType": "string",
    "enums": null,
    "comment": "Resource to subscribe to at ``rtds_config`` for the RTDS layer.",
    "notImp": false
  },
  {
    "name": "rtds_config",
