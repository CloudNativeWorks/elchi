...
export const TcpProxy: OutType = { "TcpProxy": [
  ...
  },
  {
    ...
    "fieldType": "Duration",
    "enums": null,
    "comment": "Note that if both this field and `access_log_flush_interval` are specified, the former (deprecated field) is ignored.\n\n:::attention\nThis field is deprecated in favor of `access_log_flush_interval`. \n:::",
    "notImp": false
  },
  {
    "name": "flush_access_log_on_connected",
    ...
    "fieldType": "boolean",
    "enums": null,
    "comment": "Note that if both this field and `flush_access_log_on_connected` are specified, the former (deprecated field) is ignored.\n\n:::attention\nThis field is deprecated in favor of `flush_access_log_on_connected`. \n:::",
    "notImp": false
  },
  {
  ...
  },
  {
    ...
    "fieldType": "TlvEntry[]",
    "enums": null,
    "comment": "If set, the specified PROXY protocol TLVs (Type-Length-Value) will be added to the PROXY protocol state created by the TCP proxy filter. These TLVs will be sent in the PROXY protocol v2 header to upstream.\n\nThis field only takes effect when the TCP proxy filter is creating new PROXY protocol state and there is an upstream proxy protocol transport socket configured in the cluster. If the connection already contains PROXY protocol state (including any TLVs) parsed by a downstream proxy protocol listener filter, the TLVs specified here are ignored.\n\n:::note\nTo ensure specified TLVs are allowed in the upstream PROXY protocol header, you must also configure the passthrough TLVs on the upstream proxy protocol transport. See `core.v3.ProxyProtocolConfig.pass_through_tlvs` for details.",
    "notImp": false
  }
] };
