...
const ComponentFilters: React.FC<GeneralProps> = ({ veri }) => {
    ...
    const { data: dataQuery } = useCustomGetQuery({
        queryKey: `custom_network_filters_${searchQuery}`,
        enabled: true,
        path: `custom/resource_list_search?collection=filters&category=envoy.filters.network&version=${veri.version}&project=${project}&search=${searchQuery}`
    });
