...
const ComponentCertSdsConfig: React.FC<GeneralProps> = ({ veri }) => {
    ...
    const { data: queryData } = useCustomGetQuery({
        queryKey: `custom_secrets_cert_sds_${searchQuery}`,
        enabled: true,
        path: `custom/resource_list_search?collection=secrets&gtype=envoy.extensions.transport_sockets.tls.v3.TlsCertificate&version=${veri.version}&project=${project}&search=${searchQuery}`
    });
