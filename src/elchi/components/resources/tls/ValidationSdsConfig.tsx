...
const ComponentValidationSdsConfig: React.FC<GeneralProps> = ({ veri }) => {
    ...
    const { data: queryData } = useCustomGetQuery({
        queryKey: `custom_secrets_validation_sds_${searchQuery}`,
        enabled: true,
        path: `custom/resource_list_search?collection=secrets&gtype=envoy.extensions.transport_sockets.tls.v3.CertificateValidationContext&version=${veri.version}&project=${project}&search=${searchQuery}`
    });
