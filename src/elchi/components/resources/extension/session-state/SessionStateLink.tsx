...
const ComponentSessionStateLink: React.FC<GeneralProps> = ({ veri }) => {
    ...
    const { data: queryDataHeader } = useCustomGetQuery({
        queryKey: `custom_session_state_header_${searchQuery}`,
        enabled: true,
        path: `custom/resource_list_search?collection=extensions&gtype=${GTypes.HeaderBasedSessionState}&version=${veri.version}&project=${project}&search=${searchQuery}`
    });

    const { data: queryDataCookie } = useCustomGetQuery({
        queryKey: `custom_session_state_cookie_${searchQuery}`,
        enabled: true,
        path: `custom/resource_list_search?collection=extensions&gtype=${GTypes.CookieBasedSessionState}&version=${veri.version}&project=${project}&search=${searchQuery}`
    });
