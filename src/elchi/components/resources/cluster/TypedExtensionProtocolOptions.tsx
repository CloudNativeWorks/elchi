...
const ComponentTypedExtensionProtocolOptions: React.FC<GeneralProps> = ({ veri }) => {
    ...
    const { data: queryData } = useCustomGetQuery({
        queryKey: `custom_http_protocol_options_${searchQuery}`,
        enabled: true,
        path: `custom/resource_list_search?collection=extensions&version=${veri.version}&project=${project}&category=envoy.upstreams.http.http_protocol_options&canonical_name=envoy.upstreams.http.http_protocol_options&search=${searchQuery}`
    });
