...
const EdsClusterConfig: React.FC<GeneralProps> = ({ veri }) => {
    ...
    const { data: queryData } = useCustomGetQuery({
        queryKey: "custom_endpoints",
        enabled: true,
        path: `custom/resource_list?collection=endpoints&type=endpoints&version=${veri.version}&project=${project}`
    });
    ...
    const fieldConfigs: FieldConfigType[] = [
        { tag: "service_name", type: FieldTypes.Select, placeHolder: "(string)", fieldPath: 'service_name', spanNum: 12, values: queryData ? queryData.map((item: { name: string }) => item.name) : [], required: true },
    ]
