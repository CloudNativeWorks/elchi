...
const ComponentOriginalIPDetectionExtensions: React.FC<GeneralProps> = ({ veri }) => {
    ...
    const { data: queryData } = useCustomGetQuery({
        queryKey: `custom_original_ip_detection_${searchQuery}`,
        enabled: true,
        path: `custom/resource_list_search?collection=extensions&category=envoy.http.original_ip_detection&version=${veri.version}&project=${project}&search=${searchQuery}`
    });
    ...
    const handleChangeRedux = (key: string, val: string) => {
        ...
        if (matchedExt) {
            ...
            console.error("Matched original IP detection extension not found for the given value:", val);
        }
    };
                                ...
                                <EForm>
                                    <FieldComponent
                                        veri={{
                                            ...
                                            placeholder: "(Original IP Detection Extension)",
...
export default memorizeComponent(ComponentOriginalIPDetectionExtensions, compareVeri);
