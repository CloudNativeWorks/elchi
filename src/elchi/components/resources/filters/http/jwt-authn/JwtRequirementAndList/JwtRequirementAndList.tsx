...
import { Button, Collapse, Drawer, Empty, Divider, Popconfirm } from 'antd';
...
import { modtag_jwt_requirement_and_list } from "./_modtag_";
...
const ComponentJwtRequirementAndList: React.FC<GeneralProps> = ({ veri }) => {
    const [state, setState] = useState<boolean>(false);
    const [stateActiveItem, setStateActiveItem] = useState<number>(0);
    const dispatch = useDispatch();

    const { vTags, loading } = useTags(veri.version, modtag_jwt_requirement_and_list);
    ...
    if (loading || !vTags.jral) {
        return null;
    }

    const fieldConfigs: FieldConfigType[] = [
        ...generateFields({
            f: vTags.jral?.JwtRequirementAndList,
            sf: vTags.jral?.JwtRequirementAndList_SingleFields,
        }),
    ];
    ...
    return (
        <>
            <ECard title={veri.title}>
                <HorizonTags veri={{
                    tags: vTags.jral?.JwtRequirementAndList,
                            ...
                            description={
                                <div style={{ marginTop: 16 }}>
                                    ...
                                    <p style={{
                                        ...
                                        Click the + button above to add JWT requirements. All of them must pass.
                                    </p>
                                </div>
                            }
                        />
...
export default memorizeComponent(ComponentJwtRequirementAndList, compareVeri);
