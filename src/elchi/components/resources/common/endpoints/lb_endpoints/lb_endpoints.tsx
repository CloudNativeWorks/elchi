...
const LBEndpointsComponent: React.FC<GeneralProps> = ({ veri }) => {
    ...
    const { successMessage, errorMessage, warningMessage } = useCustomMessage(messageApi);
    ...
    const handleAdd = (values: any) => {
        ...
        if (values.bulkAddress) {
            ...
            if (Array.isArray(addresses)) {
                ...
                if (fail) {
                    warningMessage("Some addresses were already on the list!");
                }

                setState((prevState) => ({ ...prevState, modal_bulk: false }));
                successMessage("Added");
            } else {
                errorMessage(addresses);
            }
        }
    };
