...
const CommonComponentRequestMirrorPolicy: React.FC<GeneralProps> = ({ veri }) => {
    ...
    const { data: queryData } = useCustomGetQuery({
        queryKey: "custom_rmp",
        enabled: true,
        path: `custom/resource_list?collection=clusters&type=clusters&version=${veri.version}&project=${project}`
    });

    const fieldConfigs: FieldConfigType[] = [
        { tag: "cluster", type: FieldTypes.Select, placeHolder: "(Cluster)", fieldPath: 'cluster', spanNum: 12, required: true, values: queryData ? queryData.map((item: { name: string }) => item.name) : [], },
        ...generateFields({
