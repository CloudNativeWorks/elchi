...
const ComponentInternalRedirectPredicates: React.FC<GeneralProps> = ({ veri }) => {
    ...
    const { data: queryData } = useCustomGetQuery({
        queryKey: `custom_internal_redirect_predicates_${searchQuery}`,
        enabled: true,
        path: `custom/resource_list_search?collection=extensions&category=envoy.internal_redirect_predicates&version=${veri.version}&project=${project}&search=${searchQuery}`
    });
    ...
    const handleChangeRedux = (key: string, val: string) => {
        ...
        if (matchedExt) {
            ...
            console.error("Matched internal redirect predicate not found for the given value:", val);
        }
    };
                                ...
                                <EForm>
                                    <FieldComponent
                                        veri={{
                                            ...
                                            placeholder: "(Internal Redirect Predicate)",
...
export default memorizeComponent(ComponentInternalRedirectPredicates, compareVeri);
