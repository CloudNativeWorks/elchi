...
const DynamicComponentBuilder: React.FC<DynamicComponentBuilderProps> = ({
    ...
    useEffect(() => {
        console.log('DynamicComponentBuilder - initialComponents changed:', initialComponents);
        setComponents(initialComponents);
    }, [initialComponents]);

    const { useGetComponentCatalog } = useScenarioAPI();
    const { data: componentCatalogData, isLoading: catalogLoading } = useGetComponentCatalog();
    ...
    const handleAddComponent = (values: any) => {
        ...
        if (newComponents.length > 0) {
            ...
            addForm.resetFields();
            message.success(`${newComponents.length} component(s) added successfully`);
        }
    };
    ...
    const handleEditClick = (index: number, newName?: string) => {
        if (newName) {
            ...
            if (components.some((c, i) => i !== index && c.name === newName)) {
                message.warning('Component name already exists');
                return;
            }
            ...
            setComponents(updatedComponents);
            message.success('Component name updated');
        } else {
    ...
    const handleFieldConfigSubmit = (values: any) => {
        if (configuringIndex >= 0) {
            ...
            if (!componentDef) {
                message.error('Component definition not found');
                return;
            }
    ...
    const handleSubmit = () => {
        ...
        if (componentsWithoutFields.length > 0) {
            message.warning(`Please configure required fields for: ${componentsWithoutFields.map(c => c.name).join(', ')}`);
            return;
        }
                ...
                <Form form={addForm} layout="vertical" onFinish={handleAddComponent}>
                    ...
                    >
                        <div style={{ maxHeight: 'calc(100vh - 200px)', overflowY: 'auto' }}>
                            {componentCatalogData?.components
