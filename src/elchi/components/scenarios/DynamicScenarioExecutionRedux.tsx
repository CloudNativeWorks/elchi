...
const DynamicScenarioExecutionRedux: React.FC = () => {
    ...
    const handleVersionSelect = (version: string, managedMode: boolean) => {
        ...
        showSuccessNotification(`Selected proxy version: ${version} (${modeText})`);
    };
    ...
    const handleExecute = async () => {
        if (!scenario || !project) {
            showErrorNotification('Scenario or project not found');
            return;
        }

        if (!selectedVersion) {
            showErrorNotification('Please select an Envoy version first');
                ...
                <Steps 
                    ...
                    onChange={(newStep) => {
                        ...
                        if (newStep < currentStep || isCurrentStepValid) {
                            ...
                            showWarningNotification('Please complete the current step before moving forward');
                        }
                    }}
                >
