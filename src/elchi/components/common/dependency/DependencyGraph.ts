...
export const createDependencyGraph = (
    ...
    try {
        ...
        try {
            ...
            // Calculate adaptive node size based on node count - consistent sizing for all cases
            const nodeCount = formattedElements.filter(el => el.group === 'nodes').length;
            const getAdaptiveNodeSize = () => {
                if (nodeCount === 1) return { width: 120, height: 80, fontSize: 16 };
                if (nodeCount <= 3) return { width: 100, height: 70, fontSize: 15 };
                if (nodeCount <= 8) return { width: 90, height: 65, fontSize: 14 };
                return { width: 80, height: 60, fontSize: 13 };
            };
            ...
            const cy = cytoscape({
                ...
                layout: nodeCount === 1 ? {
                    name: 'preset',
                    positions: function(_) {
                        return { x: 300, y: 200 }; // Center position for single node
                    },
                    fit: false,
                    animate: false,
                } : {
            ...
            setTimeout(() => {
                if (cy) {
                    try {
                        if (nodeCount === 1) {
                            // For single node, use fixed zoom and center position
                            cy.zoom(1);
                            cy.center();
                        } else {
                            // For multiple nodes, use fit
                            cy.center();
                            cy.fit();
                        }
