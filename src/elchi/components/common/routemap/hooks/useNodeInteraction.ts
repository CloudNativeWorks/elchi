...
import { RouteMapNode, SelectedNodeInfo } from '../types';
import {
    getNeighborNodeIds,
    getConnectedEdgeIds,
...
export const useNodeInteraction = () => {
    ...
    const handleNodeClick = useCallback(
        (node: RouteMapNode, allNodes: Node[], allEdges: Edge[]) => {
    ...
    const applySearchFilter = useCallback(
        (term: string, nodes: RouteMapNode[]) => {
            setSearchTerm(term);
    ...
    const applyCategoryFilter = useCallback(
        (category: string, nodes: RouteMapNode[]) => {
