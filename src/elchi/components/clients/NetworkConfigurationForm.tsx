...
import { Form, Input, Switch, Button, Card, Space, Typography, Select, InputNumber, notification, Divider } from 'antd';
...
const NetworkConfigurationForm: React.FC<NetworkConfigurationFormProps> = ({
    ...
    const handleNetworkResponse = (response: NetworkResponse) => {
        if (response.success) {
            let title = 'Network Configuration Applied Successfully';
            let description = response.message || 'Network configuration has been applied.';
            
            if (response.safely_applied) {
                description += ' Configuration was safely applied with test mode.';
            }
            
            if (response.connection_preserved) {
                description += ' Controller connection was maintained.';
            }
            
            if (response.backup_created) {
                description += ' Configuration backup was created.';
            }
            ...
            if (response.rollback_triggered) {
                errorMessage += ' Automatic rollback was performed.';
                notification.warning({
                    message: 'Network Change Failed - Rollback Performed',
                    description: response.rollback_reason || errorMessage,
                    duration: 8,
                });
            } else if (response.connection_lost) {
                errorMessage += ' Connection was lost during network change.';
                notification.error({
                    message: 'Connection Lost During Network Change',
                    description: errorMessage,
                    duration: 10,
                });
            } else if (response.test_failed) {
                errorMessage += ' Network test failed, configuration was not applied.';
                notification.error({
