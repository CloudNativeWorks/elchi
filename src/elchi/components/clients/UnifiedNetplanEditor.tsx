...
import { 
    Button, Form, Typography, Card, Space, Divider, Switch, message, Tag
} from 'antd';
...
const UnifiedNetplanEditor: React.FC<UnifiedNetplanEditorProps> = ({ 
    ...
    const handleFinish = async () => {
        ...
        try {
            ...
            NetworkSafetyHandler.handleNetworkResponse(response, 'Network configuration');
            
            if (response.success) {
                onSuccess();
            }
        } catch (error: any) {
            message.error(`Failed to apply configuration: ${error?.message || error}`);
        } finally {
            setLoading(false);
        }
    };
