...
import { Card, Row, Col, Statistic, Table, Tag, Typography, Space, Button, Popconfirm, Tooltip, message } from 'antd';
...
const BGPSummaryContent: React.FC<BGPSummaryContentProps> = ({ clientId }) => {
    ...
    const handleClearRoutes = async (neighborIp: string, soft: boolean) => {
        ...
        try {
            ...
            if (result.success) {
                message.success(`BGP routes ${soft ? 'soft' : 'hard'} cleared for ${neighborIp}`);
