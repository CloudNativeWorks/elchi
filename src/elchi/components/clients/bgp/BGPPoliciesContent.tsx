...
import { Tabs, Table, Button, Card, Modal, Popconfirm, Tag, Space } from 'antd';
...
const BGPPoliciesContent: React.FC<BGPPoliciesContentProps> = ({ clientId }) => {
    ...
    const handleBatchRemove = (type: 'routemap' | 'community' | 'prefix') => {
        ...
        if (blockedItems.length > 0) {
            showErrorNotification(`Cannot delete the following ${type}s because they are being used in route maps: ${blockedItems.join(', ')}`);
            return;
        }
                ...
                <Space size="small">
                    ...
                    <Popconfirm
                        ...
                        onConfirm={() => {
                            const usedInRouteMaps = isCommunityListUsedInRouteMaps(record.name);
                            if (usedInRouteMaps.length > 0) {
                                showErrorNotification(`Cannot delete community list "${record.name}". It is being used in route maps: ${usedInRouteMaps.join(', ')}`);
                                return;
                            }
                ...
                <Space size="small">
                    ...
                    <Popconfirm
                        ...
                        onConfirm={() => {
                            const usedInRouteMaps = isPrefixListUsedInRouteMaps(record.name);
                            if (usedInRouteMaps.length > 0) {
                                showErrorNotification(`Cannot delete prefix list "${record.name}". It is being used in route maps: ${usedInRouteMaps.join(', ')}`);
                                return;
                            }
