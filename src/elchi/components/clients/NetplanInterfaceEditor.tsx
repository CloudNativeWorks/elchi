...
import { 
    ...
    message, Card, Space, Divider, InputNumber, Select
} from 'antd';
...
const NetplanInterfaceEditor: React.FC<NetplanInterfaceEditorProps> = ({ 
    ...
    const [routingTable, setRoutingTable] = useState<number>(254); // Default to main table
    const [routes, setRoutes] = useState<Array<{to: string, via: string, metric?: number}>>([]);
    ...
    useEffect(() => {
        ...
        if (currentInterface) {
            ...
            setDhcp4(false);
            setAddresses(currentInterface.addresses || []);
            setMtu(currentInterface.mtu || 1500);
    ...
    const handleFinish = async (values: any) => {
        ...
        try {
            ...
            NetworkSafetyHandler.handleNetworkResponse(response, `Interface ${currentInterface.name} configuration`);
            
            if (response.success) {
                onSuccess();
            }
        } catch (error: any) {
            message.error(`Failed to update interface: ${error?.message || error}`);
        } finally {
            setLoading(false);
        }
    };
