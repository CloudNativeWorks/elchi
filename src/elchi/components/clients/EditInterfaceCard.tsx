...
import { Button, Form, Input, Typography, Row, Col, Divider, Checkbox } from 'antd';
...
const EditInterfaceCard: React.FC<EditInterfaceCardProps> = ({
    ...
    const getInterfaceTable = (ifname: string) => {
        ...
        return found ? found.id : 254; // default main table
    };
    ...
    const handleFinish = async (values: any) => {
        if (!values.address) {
            showErrorNotification('IP address is required');
            return;
        }
        ...
        try {
            ...
            if (!networkSubnet) {
                showErrorNotification('Invalid IP address format');
                return;
            }
            ...
            showSuccessNotification('Interface configuration saved successfully');
            setEditIndex(null);
        } catch (error: any) {
            showErrorNotification(error, 'Failed to save interface configuration');
        } finally {
            setLoading(false);
        }
    };
